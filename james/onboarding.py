...
def write_env(
    ...
    *,
    ...
    """Write a complete, safe ``.env`` file. Returns its path.

    Shared by the interactive wizard, the web onboarding wizard, and tests.
    Secrets only ever land in ``.env`` (gitignored, chmod 600 on POSIX).
    """
    ...
    lines.append(f"VOICE_ENABLED={'true' if voice_enabled else 'false'}")
...
def configure(
    ...
    *,
    ...
    """Non-interactive configuration for the web onboarding wizard."""
    return write_env(provider, model, api_key, voice_enabled=voice_enabled, base_url=base_url)
